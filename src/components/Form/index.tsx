  ...
  return (
    <form onSubmit={handleSubmit} data-testid="form">
      <Flex justify="space-between" direction="column" gap="1.5rem">
        <Box flex="1">
          <CartValueInput
            onDataValidityChange={(isValid) =>
              handleDataValidityChange("cartValue", isValid)
            }
          />
        </Box>
        <Box flex="1">
          <DistanceInput
            onDataValidityChange={(isValid) =>
              handleDataValidityChange("distance", isValid)
            }
          />
        </Box>
        <Box flex="1">
          <NumOfItemsInput
            onDataValidityChange={(isValid) =>
              handleDataValidityChange("numOfItems", isValid)
            }
          />
        </Box>
        <Box flex="1">
